...
with col2:
    start_date = st.date_input(
        "Start Date", datetime.today() - timedelta(days=5 * 365)
    )
with col3:
    ...
    if start_date > end_date:
        ...
        try:
            ...
            if hist.empty:
                ...
                data["High%"] = (
                    data["High"] / data["Close"].shift(1) - 1
                ) * 100
                data["Low▲"] = data["Low"] - data["Close"].shift(1)
                data["Low%"] = (
                    data["Low"] / data["Close"].shift(1) - 1
                ) * 100
                ...
                with col1:
                    st.write(
                        f"Final RSI (30,70): {data['RSI'].iloc[-1]:.2f}"
                    )
                    st.write(
                        f"Final MFI (20,80): {data['MFI'].iloc[-1]:.2f}"
                    )
                with col2:
                    st.write(
                        f"ATR (14d): {data['ATR'].iloc[-1]:.2f} ({(data['ATR'].iloc[-1] / current_price * 100):.2f}%)"
                    )
                    st.write(
                        f"Volatility (30d): {data['Volatility'].iloc[-1]:.2f} ({(data['Volatility'].iloc[-1] / current_price * 100):.2f}%)"
                    )
                with col3:
                    st.write(
                        f"High: {float(data['High'].max()):.2f}"
                    )
                    st.write(
                        f"Low: {float(data['Low'].min()):.2f}"
                    )
                ...
                if not dividend_dates.empty:
                    fig.add_trace(
                        go.Scatter(
                            ...
                            mode="markers",
                            marker=dict(
                                symbol="star", size=10, color="green"
                            ),
                            name="Dividend Payout",
                            text=[
                                f"Dividend: ${d:.2f}"
                                for d in dividend_dates["Dividends"]
                            ],
                            hoverinfo="text+x+y",
                        )
                    )
                ...
                fig2.add_hline(
                    y=70, line=dict(color="red", dash="dot")
                )
                fig2.add_hline(
                    y=30, line=dict(color="green", dash="dot")
                )
                ...
                fig2.add_hline(
                    y=80, line=dict(color="red", dash="dot")
                )
                fig2.add_hline(
                    y=20, line=dict(color="green", dash="dot")
                )

                fig2.update_layout(
                    title="RSI and MFI Indicators",
                    xaxis_title="Date",
                    yaxis_title="Value",
                )
                ...
                with c1:
                    st.write(
                        f"Ave Open▲: ${float(data['Open▲'].mean()):.2f}"
                    )
                    st.write(
                        f"Ave Open%: {float(data['Open%'].mean()):.2f}%"
                    )
                with c2:
                    st.write(
                        f"Ave High▲: ${float(data['High▲'].mean()):.2f}"
                    )
                    st.write(
                        f"Ave High%: {float(data['High%'].mean()):.2f}%"
                    )
                with c3:
                    st.write(
                        f"Ave Low▲: ${float(data['Low▲'].mean()):.2f}"
                    )
                    st.write(
                        f"Ave Low%: {float(data['Low%'].mean()):.2f}%"
                    )
                with c4:
                    st.write(
                        f"Ave Close▲: ${float(data['Close▲'].mean()):.2f}"
                    )
                    st.write(
                        f"Ave Close%: {float(data['Close%'].mean()):.2f}%"
                    )
                ...
                show_only_dividends = st.checkbox(
                    "Show only rows with dividends > 0"
                )
