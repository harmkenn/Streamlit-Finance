...
# Initialize portfolio positions in session state if not already present
if "portfolio_positions" not in st.session_state:
    st.session_state["portfolio_positions"] = {}

# Merge user positions into dataframe
df["Shares Owned"] = df["Symbol"].apply(lambda s: st.session_state["portfolio_positions"].get(s, {}).get("shares", 0.0))
df["Avg Cost"] = df["Symbol"].apply(lambda s: st.session_state["portfolio_positions"].get(s, {}).get("cost", 0.0))
df["Profit"] = df.apply(lambda r: calculate_position_profit(r["Price"], r["Avg Cost"], r["Shares Owned"]), axis=1)

# Format display columns
overview_df = df[["Symbol_URL", "Symbol", "Price", "Change", "% Change", "Shares Owned", "Avg Cost", "Profit", "1M %", "YTD %"]].copy()

st.subheader("📋 Live Prices & Positions Tracker")

# Single interactive Data Editor
